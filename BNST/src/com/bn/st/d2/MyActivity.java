...
public class MyActivity extends Activity {
	...
	private SensorEventListener mySensorListener = new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
			...
			if (event.sensor.getType() == Sensor.TYPE_ORIENTATION) {// 判断是否为加速度传感器变化产生的数据
																	// 计算出重力在屏幕上的投影方向
				int[] directionDotXY = RotateUtil.getDirectionDot(event.values);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		com.bn.clp.Constant.SoundEffectFlag = sp.getBoolean("soundEffectFlag",
				true);
		...
		if (Math.abs(com.bn.clp.Constant.screenRatio
				- com.bn.clp.Constant.screenRatio854x480) < 0.001f) {
			com.bn.clp.Constant.screenId = 1;
		} else if (Math.abs(com.bn.clp.Constant.screenRatio
				- com.bn.clp.Constant.screenRatio480x320) < 0.01f) {
			com.bn.clp.Constant.screenId = 3;
		} else if (Math.abs(com.bn.clp.Constant.screenRatio
				- com.bn.clp.Constant.screenRatio960x540) < 0.001f) {
		...
		new Thread() {
			public void run() {
				synchronized (boatInitLock) {
					...
					XCSurfaceView.loadWelcomeBitmap(MyActivity.this
							.getResources());
					XCSurfaceView.loadVertexFromObj(MyActivity.this
							.getResources());
					com.bn.st.xc.ShaderManager.loadCodeFromFile(MyActivity.this
							.getResources());
					...
					MyGLSurfaceView.loadProgressBitmap(MyActivity.this
							.getResources());
					com.bn.clp.ShaderManager.loadCodeFromFile(MyActivity.this
							.getResources());
				}
			}
		}.start();
	...
	public void onPrepareDialog(int id, Dialog dialog) {
		...
		case 4:
			...
			tv.setText("恭喜您，您获得了第" + com.bn.clp.Constant.RANK_FOR_HERO_BOAT
					+ "名。");
			ok.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
