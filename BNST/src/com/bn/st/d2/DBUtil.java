...
public class DBUtil {
	...
	public static void createorOpenDatabase() {
		try {
			sld = SQLiteDatabase.openDatabase("/data/data/com.bn/mydb", // 数据库所在路径
					null, // CursorFactory
	...
	public static void createTable() {
		...
		try {
			String jsSql = "create table if not exists jsRecord" + "("
					...
					+ "date char(20)," + "time char(10)" + ")";
			String rcRSql = "create table if not exists jRecord" + "("
					+ "id INTEGER PRIMARY KEY AUTOINCREMENT,"
					+ "date char(20)," + "time char(10)," + "ranking Integer"
					+ ")";
			sld.execSQL(jsSql);
			sld.execSQL(rcRSql);
		} catch (Exception e) {
	...
	public static void insertJSDatabase(String date, String time) {
		createorOpenDatabase();
		try {
			String sql = "insert into jsRecord values(null, '" + date + "','"
					+ time + "');";
			sld.execSQL(sql);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void insertRcRDatabase(String date, String time, int ranking) {
		createorOpenDatabase();
		try {
			String sql = "insert into jRecord values(null, '" + date + "','"
					+ time + "','" + ranking + "');";
			sld.execSQL(sql);
		} catch (Exception e) {
			e.printStackTrace();
