...
public class Constant {
	//
	public static boolean BgSoundFlag;//
	public static boolean SoundEffectFlag;//

	//
	public static int screenId = 0;
	public static float screenRatio;

	public static final float screenRatio480x320 = 1.5f;//
	...
	public static final float[][] XC_Self_Adapter_Data_TRASLATE = {// 上一个 下一个
																	// 按钮高度的一半
	{ 0.21f }, { 0.22f }, { 0.22f }, { 0.20f } };
	...
	// 设定房间的颜色
	public static final float[][] HOUSE_COLOR = new float[][] { { 1f, 1f, 1f },// 不透明地面
			{ 1f, 1f, 1f },// 透明墙
	};
	// 设定展台圆柱颜色
	public static final float[] COLOR_CYLINDER = new float[] { 0.9f, 0.9f,
			0.9f, 1.0f };
	// 设定展台圆面颜色
	public static final float[] COLOR_CIRCLE = new float[] { 0.9f, 0.9f, 0.9f,
			0.5f };
	// 围墙的宽高 这里的宽高均为 实际的一半
	...
	// 需要的图片的id
	public static int[] picId = new int[] { R.drawable.background, // 大背景
	...
	// 数字图片
	public static int[] picNum = new int[] { R.drawable.d0, R.drawable.d1,
	...
	public static void initBitmap(Resources res) {
		...
		for (int i = 0; i < picId.length; i++) {
			recordBitmap[i] = scaleToFit(
					BitmapFactory.decodeResource(res, picId[i]), ratio_width,
					ratio_height);
		}
		for (int i = 0; i < picNum.length; i++) {
			recordNum[i] = scaleToFit(
					BitmapFactory.decodeResource(res, picNum[i]), ratio_width,
					ratio_height);
		}
	}
	...
	public static void initLoaction() {
		// 图片的位置信息
		float[][] tempPicLocation = new float[][] { { 0, 0 },// 大背景图片位置
				{ (SCREEN_WIDTH - recordBitmap[1].getWidth()) / 2, 150 },// 计时模式图片位置
		};
		// 可触控区间
		float[][] tempTouchLocation = new float[][] {
				{ (SCREEN_WIDTH - recordBitmap[1].getWidth()) / 2, 150,
						(SCREEN_WIDTH) / 2, 200 },// Timing Mode的位置
				{ (SCREEN_WIDTH) / 2, 150,
						(SCREEN_WIDTH + recordBitmap[1].getWidth()) / 2, 200 },// Racing
																				// Mode的位置
				{ (SCREEN_WIDTH - recordBitmap[1].getWidth()) / 2, 230,
						(SCREEN_WIDTH + recordBitmap[1].getWidth()) / 2, 410 },// 绘制时间信息的位置
		};
